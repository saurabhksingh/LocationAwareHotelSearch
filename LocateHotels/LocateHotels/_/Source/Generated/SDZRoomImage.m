/*
	SDZRoomImage.h
	The implementation of properties and methods for the SDZRoomImage object.
	Generated by SudzC.com
*/
#import "SDZRoomImage.h"

@implementation SDZRoomImage
	@synthesize roomTypeCode = _roomTypeCode;
	@synthesize url = _url;

	- (id) init
	{
		if(self = [super init])
		{
			self.roomTypeCode = nil;
			self.url = nil;

		}
		return self;
	}

	+ (SDZRoomImage*) createWithNode: (CXMLNode*) node
	{
		if(node == nil) { return nil; }
		return (SDZRoomImage*)[[[SDZRoomImage alloc] initWithNode: node] autorelease];
	}

	- (id) initWithNode: (CXMLNode*) node {
		if(self = [super initWithNode: node])
		{
			self.roomTypeCode = [Soap getNodeValue: node withName: @"roomTypeCode"];
			self.url = [Soap getNodeValue: node withName: @"url"];
		}
		return self;
	}

	- (NSMutableString*) serialize
	{
		return [self serialize: @"RoomImage"];
	}
  
	- (NSMutableString*) serialize: (NSString*) nodeName
	{
		NSMutableString* s = [NSMutableString string];
		[s appendFormat: @"<%@", nodeName];
		[s appendString: [self serializeAttributes]];
		[s appendString: @">"];
		[s appendString: [self serializeElements]];
		[s appendFormat: @"</%@>", nodeName];
		return s;
	}
	
	- (NSMutableString*) serializeElements
	{
		NSMutableString* s = [super serializeElements];
		if (self.roomTypeCode != nil) [s appendFormat: @"<roomTypeCode>%@</roomTypeCode>", [[self.roomTypeCode stringByReplacingOccurrencesOfString:@"\"" withString:@"&quot;"] stringByReplacingOccurrencesOfString:@"&" withString:@"&amp;"]];
		if (self.url != nil) [s appendFormat: @"<url>%@</url>", [[self.url stringByReplacingOccurrencesOfString:@"\"" withString:@"&quot;"] stringByReplacingOccurrencesOfString:@"&" withString:@"&amp;"]];

		return s;
	}
	
	- (NSMutableString*) serializeAttributes
	{
		NSMutableString* s = [super serializeAttributes];

		return s;
	}
	
	-(BOOL)isEqual:(id)object{
		if(object != nil && [object isKindOfClass:[SDZRoomImage class]]) {
			return [[self serialize] isEqualToString:[object serialize]];
		}
		return NO;
	}
	
	-(NSUInteger)hash{
		return [Soap generateHash:self];

	}
	
	- (void) dealloc
	{
		self.roomTypeCode = nil;
		self.url = nil;
		[super dealloc];
	}

@end
